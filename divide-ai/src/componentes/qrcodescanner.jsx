  ...
  const handleQrCodeReaded = async (err, result) => {
    if (result?.text) {
      ...
      try {
        ...
        navigate("/table", {
          state: { url, purchaseData, classifyItems: true },
        });
      } catch (error) {
    ...
    <Box>
      ...
      >
        <DialogTitle sx={{ display: "flex", gap: 1 }}>
          <p
            style={{ fontFamily: "'Jersey 15'", fontSize: 32, color: "white" }}
          >
            Tempo Esgotado
          </p>
          <img
        ...
        <DialogContent>
          <p style={{ fontFamily: "'Roboto'", color: "white" }}>
            Não foi possível identificar um QR Code no tempo estimado ou a nota
            fiscal é inválida.
          </p>
          <br />
          <p style={{ fontFamily: "'Roboto'", color: "white" }}>
